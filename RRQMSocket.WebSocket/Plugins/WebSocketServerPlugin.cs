    ...
    public class WebSocketServerPlugin : HttpPluginBase
    {
        ...
        protected override void OnGet(ITcpClientBase client, HttpContextEventArgs e)
        {
            if (this.WSUrl == "/" || e.Context.Request.UrlEquals(this.WSUrl))
            {
                ...
                {
                    e.Handled = true;
                    if (WSTools.TryGetResponse(e.Context.Request, out HttpResponse response))
                    {
                        HttpContextEventArgs args = new HttpContextEventArgs(new HttpContext(e.Context.Request, response))
                        {
                        ...
                        if (args.IsPermitOperation)
                        {
                            ...
                            this.PluginsManager.Raise<IWebSocketPlugin>("OnHandshaked", client, new HttpContextEventArgs(e.Context));
                        }
                        else
                        {
